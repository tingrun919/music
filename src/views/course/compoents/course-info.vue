...
<template>
	<div>
		<div class="course-bg" v-bind:style="{backgroundImage: 'url(' + img + ')'}">
			<div>
				<p v-if="courseList">{{courseList.aClbetime}}</p>
			</div>
			<div>
				<h3 v-if="courseList">导师：{{courseList.aNickname}}</h3>
				<!-- <span>2375</span> -->
				<!-- <span>213</span> -->
				<!-- <span>1203</span> -->
			</div>
			...
			<v-container grid-list-md text-xs-center>
				<v-layout row wrap v-bind="binding">
					<v-flex xs8 offset-xs2>
						<div v-if="courseList" v-html="courseList.aJiangyi"></div>
						<!-- <h2>萨克斯初级-入门教学第二节</h2>
...
<script>
	...
	export default {
		...
		mounted() {
			setTimeout(() => {
				this.playerOptions.sources[0].src = this.videoList.origUrl;
			}, 500)
